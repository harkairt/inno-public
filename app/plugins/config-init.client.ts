...
function sanitizeCssValue(value: string, type: 'color' | 'style'): string {
  if (type === 'color') {
    const colorPattern = /^(#[0-9a-fA-F]{3,8}|rgb\(\d+,\s*\d+,\s*\d+\)|rgba\(\d+,\s*\d+,\s*\d+,\s*[\d.]+\)|[a-zA-Z]+)$/
    return colorPattern.test(value.trim()) ? value.trim() : ''
  }
  if (type === 'style') {
    const allowed = ['none', 'solid', 'dashed', 'dotted', 'double', 'groove', 'ridge', 'inset', 'outset']
...
function applyConfigCssVariables(config: InnoChatConfig): void {
  ...
  // Own message styling
  root.style.setProperty('--config-own-message-bg', sanitizeCssValue(config.ownMessageBackgroundColor, 'color'))
  ...
  root.style.setProperty('--config-partner-message-bg', sanitizeCssValue(config.partnerMessageBackgroundColor, 'color'))
  ...
  root.style.setProperty('--config-message-border-color', sanitizeCssValue(config.messageBorderColor, 'color'))
  root.style.setProperty('--config-message-border-style', sanitizeCssValue(config.messageBorderStyle, 'style'))
  ...
  root.style.setProperty('--config-background-color', sanitizeCssValue(config.backgroundColor, 'color'))
}
